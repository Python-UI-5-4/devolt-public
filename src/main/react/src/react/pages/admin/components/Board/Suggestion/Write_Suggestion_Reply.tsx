...
import { SuggestionReplyRequest } from '../../../../../../api/AxiosApi/AdminApi/AdminApiTypes';
...
import { SuggestionReplyEditorProps } from '../../../AdminType';
...
const Write_Suggestion_Reply: React.FC<SuggestionReplyEditorProps> = ({ handleCloseEditor }) => {
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    try {
      const request: SuggestionReplyRequest = {
        suggestionId: Number(boardId),
        content: editor?.getHTML() ?? '',
      };

      const response = await AdminApi.writeSuggestionReply(request);
      if (response.data) {
        setDialogMessage('답글이 성공적으로 작성되었습니다.');
        setDialogOpen(true);
      }
    } catch (error) {
    ...
    <>
      ...
      >
        ...
        <DialogActions sx={{ justifyContent: 'right' }}>
          ...
          ) : dialogMessage === '답글이 성공적으로 작성되었습니다.' ? (
            <Button
              sx={{ fontFamily: 'bold' }}
              color="secondary"
              onClick={() => {
...
export default Write_Suggestion_Reply;
