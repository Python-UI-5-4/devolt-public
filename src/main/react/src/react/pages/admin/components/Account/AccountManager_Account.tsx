...
import { PwRequest } from '../../../../../api/AxiosApi/AdminApi/AdminApiTypes';
import {
  RightContainerEach,
...
const AccountManager_Account = (): JSX.Element => {
  ...
  const handlePasswordCheck = async (): Promise<void> => {
    try {
      const data: PwRequest = { password: currentPw }; // 객체로 담긴 pw 를 data 변수로 선언
      const isMatch = await AdminApi.validatePw(data || ''); // null이든 빈 문자열이든 그대로 요청 전송
      if (isMatch) {
      ...
      if (error) setMessage('비밀번호 확인 중 오류가 발생했습니다.');
    }
  };
  ...
  const handlePasswordChange = async (): Promise<void> => {
    ...
    try {
      const data: PwRequest = { password: newPw };
      const success = await AdminApi.modifyPw(data);
      if (success) {
      ...
      if (error) setMessage('비밀번호 변경 중 오류가 발생했습니다.');
    }
  };
