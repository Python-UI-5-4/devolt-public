...
import { ModifyCoursePostRequset } from '../../../../../api/AxiosApi/CommunityApi/CommunityApiType';
...
import { PostModifyEditorCourseProps } from '../../CommunityType';
...
const Post_ModifyEditor_Course: React.FC<PostModifyEditorCourseProps> = ({
  boardId,
  content,
  title,
  course,
...
- 서비스 운영 관련 문의는 홈페이지 우측 CS 메뉴를 이용해주세요.`,
  ...
  const handleModify = async (): Promise<void> => {
    ...
    try {
      const data: ModifyCoursePostRequset = {
        boardType,
      ...
      const response = await CommunityApi.modifyCoursePost(data);
      if (response.data) {
