...
import { BoardSearchProps, CourseType } from '../../CommunityType';
import { CourseDisplayNames } from '../common/DisplayNames';

const Board_Course_Search: React.FC<BoardSearchProps> = ({
  onSearchChange,
  onEnumFilterRefresh,
  enumFilter,
}) => {
      ...
      <SearchContainer>
        ...
        <TagSearchContainer>
          <TagSearchBox theme={theme.palette.mode}>
            <TagSearch>
              {enumFilter !== null && (
                <TagSearchItem>{CourseDisplayNames[enumFilter as CourseType]}</TagSearchItem>
              )}
            </TagSearch>
          </TagSearchBox>
          <ResetButtonBox
