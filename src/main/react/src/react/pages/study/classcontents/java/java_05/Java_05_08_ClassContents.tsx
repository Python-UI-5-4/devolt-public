    ...
    <>
      <EachClass>
        ...
        <ClassContentsContainer>
          ...
          <ClassContentsText>
            java.util 패키지에는 프로그램을 개발하는 데 사용할 수 있는 유용한 유틸리티 클래스가 다수
            포함되어 있습니다.
            <br />
            실제로 java.lang 패키지 다음으로 가장 많이 사용되는 패키지가 java.util 패키지입니다.
            <br />
            하지만 import 문을 사용하지 않아도 바로 사용할 수 있는 java.lang 패키지와는 달리
            java.util 패키지는 <b>import 문으로 패키지를 불러오고 나서야 클래스 이름만으로 사용</b>
            할 수 있습니다.
          </ClassContentsText>
          <br />
          <ClassContentsTitle2>java.util.Arrays 클래스</ClassContentsTitle2>
          <ClassContentsText>
            Arrays 클래스에는 <b>배열을 다루기 위한 다양한 메소드가 포함</b>되어 있습니다.
          </ClassContentsText>
          <br />
          <ClassContentsTitle2>binarySearch() 메소드</ClassContentsTitle2>
          <ClassContentsText>
            binarySearch() 메소드는 전달받은 배열에서 특정 객체의 위치를 이진 검색 알고리즘을
            사용하여 검색한 후, 해당 위치를 반환합니다.
            <br />이 메소드는 이진 검색 알고리즘을 사용하므로, 매개변수로 전달되는 배열이 sort()
            메소드 등을 사용하여 미리 정렬되어 있어야만 제대로 동작합니다.
            <Java_05_08_Code01 />
          </ClassContentsText>
          <br />
          ...
          <ClassContentsText>
            익명의 객체를 만드는 방법으로 Comparator의 compare 메서드를 오버라이딩 해서 정렬 조건을
            만듭니다.
            <ClassContentsCodeBox>
              💡 자바의 <ClassContentsCode>Comparator</ClassContentsCode> 인터페이스는 객체들을
              특정한 순서로 비교하는 데 사용되는 메소드를 정의합니다. 이 인터페이스는 주로 자바
              컬렉션 프레임워크에서 객체들을 정렬할 때 사용되며, 사용자가 정의한 객체의 비교 방식을
              커스터마이징할 수 있게 해줍니다.
          ...
          <ClassContentsText>
            ...
            <ClassContentsCodeBox>
              👉 <b>compareTo()</b> 함수는 두개의 값을 비교하여 int 값으로 반환해주는 함수이며 주로
              정렬에서 사용 됩니다.
              <ClassContentsTextTab>
                - <b>0</b> : 두 객체가 같음을 의미합니다.
                <br />- <b>양수</b> : 호출 객체가 매개변수로 전달된 객체보다 크다는 것을 의미합니다.
                <br />- <b>음수</b> : 호출 객체가 매개변수로 전달된 객체보다 작다는 것을 의미합니다.
              </ClassContentsTextTab>
            </ClassContentsCodeBox>
            <ClassContentsCodeBox>
              ...
              <ClassContentsTextTab>
                - <b>0</b> : 두 객체가 같음을 의미합니다.
                <br />- <b>양수</b> : 첫 번째 객체가 두 번째 객체보다 크다는 것을 의미합니다.
                <br />- <b>음수</b> : 첫 번째 객체가 두 번째 객체보다 작다는 것을 의미합니다.
              </ClassContentsTextTab>
            </ClassContentsCodeBox>
            <Java_05_08_Code04 />
          </ClassContentsText>
          ...
          <ClassContentsText>
            배열의 갯수를 입력 받아서 배열의 길이 만큼의 랜덤수를 생성해서 배열에 추가 합니다.
            <br />
            임의의 수를 입력 받아서 배열내에 해당 값이 존재하는 지 확인 합니다.
            <br />
            임의의 수는 1에서 배열의 크기 만큼의 임의의 수이지만 입력 받은 찾을 수가 배열내에 존재
            하지 않을 수 있습니다. (존재하지 않으면 음수 출력)
            <Java_05_08_Code05 />
            <ClassContentsCodeBox>
              💡 <b>이분검색이란?</b>
              <br />
              정렬된 배열에서 특정한 값을 빠르게 찾는 알고리즘입니다. 배열 내에서 중간 값과 찾으려는
              값을 비교하면서 범위를 절반씩 줄여나가는 방식으로 동작합니다. 이분 검색은 선형
              검색(Linear Search)과 비교하여 훨씬 효율적인 방법으로 원하는 값을 찾을 수 있습니다.
              <Java_05_08_Code06 />
            </ClassContentsCodeBox>
          </ClassContentsText>
        </ClassContentsContainer>
